...
import { characterApi, tierApi } from "../lib/services";
import type { CharacterListResponse, TierMeResponse } from "../lib/types";
import { ApiError } from "../lib/api";
import { formatPoint } from "../lib/format";

const FALLBACK_BG = ["#E8EAF0", "#FFF3E8", "#E8F5E9", "#EDE7F6", "#FCE4EC"];
const FALLBACK_GLOW = [
...
export default function CharacterPage() {
  ...
  const [list, setList] = useState<CharacterListResponse | null>(null);
  ...
  const load = async () => {
    const results = await Promise.allSettled([characterApi.list(), tierApi.me()]);
    if (results[0].status === "fulfilled") setList(results[0].value);
    if (results[1].status === "fulfilled") setTierMe(results[1].value);
  };
  ...
  const characters = useMemo(() => {
    const items = list?.characters ?? [];
    return items.map((c, i) => ({
      ...c,
      bgColor: FALLBACK_BG[i % FALLBACK_BG.length],
      glowColor: FALLBACK_GLOW[i % FALLBACK_GLOW.length],
      img: c.imageUrl || defaultChar,
    }));
  }, [list]);
  ...
  const selectedChar = characters.find((c) => c.id === selectedId) || activeChar;
  const coins = list?.coin ?? 0;
  const isOwned = !!selectedChar?.owned;
  const isSameAsActive = !!selectedChar?.active;
  const canAfford = selectedChar ? coins >= selectedChar.coinPrice : false;
  ...
  const handleCTA = async () => {
    ...
    try {
      ...
      if (!isOwned) {
        if (!canAfford) return;
        await characterApi.unlock(selectedChar.id);
        await characterApi.setActive(selectedChar.id);
  ...
  const ctaLabel = () => {
    ...
    if (!isOwned) return canAfford ? `${selectedChar.coinPrice}콩으로 해금하기` : "콩이 부족해요";
    if (isSameAsActive) return "현재 사용 중인 캐릭터";
    return "이 캐릭터로 변경하기";
  };

  const ctaDisabled = busy || !selectedChar || (!isOwned && !canAfford) || isSameAsActive;
    ...
    <div className="pb-28 bg-[#F5F5F5] min-h-full">
        ...
        <div className="flex items-center justify-between">
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-[16px] font-bold" style={{ color: tier.color }}>
                {tierMe?.tierLabel || tier.name}
              </span>
            </div>
            <p className="text-[13px] text-[#3B82F6] font-medium">
              {formatPoint(point)}
              {tierMe?.nextTier && " · 다음 티어 진행 중"}
            </p>
          </div>
          <button
      ...
      {activeChar && (
        <div className="flex flex-col items-center pt-7 pb-5">
          ...
          <span
            className="text-[12px] font-semibold px-3 py-1.5 rounded-full mt-2 flex items-center gap-1.5"
            style={{ backgroundColor: tier.bg, color: tier.color }}
          >
            <img src={tier.image} alt={tier.name} className="w-4 h-4 object-contain" />
            {tierMe?.tierLabel || tier.name} · {formatPoint(point)}
          </span>
        </div>
      )}
        ...
        >
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-[17px] font-bold text-[#1A1A2E] leading-tight">{coins.toLocaleString("ko-KR")}</p>
            </div>
          </div>
          <span
      ...
      <div className="px-4">
        ...
        {loading && <div className="py-8 text-center text-[13px] text-[#8E8E93]">불러오는 중...</div>}
        {!loading && characters.length === 0 && (
          <div className="py-8 text-center text-[13px] text-[#8E8E93]">사용할 수 있는 캐릭터가 없어요</div>
        )}
        ...
        <div className="grid grid-cols-2 gap-3">
          {characters.map((char) => {
            ...
            const affordable = coins >= char.coinPrice;
            const locked = !owned;
            ...
            return (
              <button
                ...
                style={{
                  border: isSelected ? "2px solid #00D26A" : "1.5px solid #F0F0F0",
                }}
              >
                {isActive && (
                ...
                >
                  {locked && !affordable && <div className="absolute inset-0 bg-white/40 z-10" />}
                  <img src={char.img} alt={char.name} className="w-[78%] h-[78%] object-contain drop-shadow-lg" />
                </div>
                ...
                <div className="mt-3 flex flex-col items-center gap-0.5 px-2">
                  ...
                  <span className="text-[11px] text-[#8E8E93] font-medium">
                    {owned ? "보유 중" : `${char.coinPrice}콩으로 해금`}
                  </span>
                </div>
      ...
      <div className="px-4 mt-5">
        {error && <p className="text-[12px] text-[#FF3B30] mb-2">{error}</p>}
        <button
